...
const EditAddress = ({ close, data }) => {
    ...
    return (
        <section className='fixed top-0 bottom-0 right-0 left-0 bg-neutral-800/80 z-50 h-screen overflow-auto'>
            <div className='bg-white p-4 w-full max-w-lg mt-8 mx-auto rounded'>
                <div className='flex justify-between items-center gap-4'>
                    <h2 className='font-semibold'>Edit Address</h2>
                    <button onClick={close} className='cursor-pointer hover:text-red-500'><IoIosCloseCircle size={22} /></button>
                </div>
                <form className='mt-4 grid gap-4' onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <button type='submit' className='bg-green-600 w-full py-2 font-semibold mt-4 hover:bg-green-700 cursor-pointer rounded-2xl'>Submit</button>
                </form>
            </div>
        </section>
    )
